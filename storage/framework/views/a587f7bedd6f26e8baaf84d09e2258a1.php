...
<?php $__env->startSection('actions'); ?>
    <a href="<?php echo e(route('checkout.index')); ?>" class="inline-block rounded bg-moss px-4 py-2 text-sm font-semibold text-white hover:bg-moss/90">
        Checkout Manual
    </a>
...
<?php $__env->startSection('content'); ?>
    <div class="grid gap-4 sm:grid-cols-3">
        <div class="rounded-lg border border-gray-200 bg-white p-4">
            ...
            <p class="mt-1 text-2xl font-bold text-ink"><?php echo e((int) ($orderSummary->total ?? 0)); ?></p>
        </div>
        <div class="rounded-lg border border-gray-200 bg-white p-4">
            <p class="text-sm text-gray-600">Berhasil</p>
            <p class="mt-1 text-2xl font-bold text-moss"><?php echo e((int) ($orderSummary->paid ?? 0)); ?></p>
        </div>
        <div class="rounded-lg border border-gray-200 bg-white p-4">
            <p class="text-sm text-gray-600">Menunggu</p>
            <p class="mt-1 text-2xl font-bold text-yellow-600"><?php echo e((int) ($orderSummary->pending ?? 0)); ?></p>
        </div>
    </div>
    ...
    <section class="mt-6">
        <div class="mb-4 flex items-end justify-between gap-3">
            ...
            <a href="<?php echo e(route('checkout.index')); ?>" class="text-sm font-semibold text-moss hover:underline">Pesan manual</a>
        </div>
                ...
                <article class="flex overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm sm:block">
                    ...
                    <div class="min-w-0 flex-1 p-3">
                        ...
                        <p class="truncate text-xs text-gray-500"><?php echo e($item->code); ?> &middot; <?php echo e($item->category->name ?? '-'); ?></p>
                            ...
                            <span class="shrink-0 rounded bg-moss/10 px-2 py-1 text-xs font-semibold text-moss"><?php echo e($item->stock); ?></span>
                        </div>
                    ...
                    <tbody>
                        <?php $__currentLoopData = $orders; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $order): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr class="border-b last:border-0">
                                ...
                                <td class="px-4 py-3 text-sm text-gray-600"><?php echo e($order->created_at->format('d M Y H:i')); ?></td>
                                <td class="px-4 py-3 text-sm">
                                    <?php if($order->status === 'pending' && $order->payment_url): ?>
                                        <a href="<?php echo e($order->payment_url); ?>" class="font-semibold text-moss hover:underline">Bayar</a>
                                    <?php else: ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\inventori\resources\views\user\dashboard.blade.php ENDPATH**/ ?>