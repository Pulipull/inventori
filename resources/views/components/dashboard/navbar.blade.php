...
<header class="sticky top-0 z-20 border-b border-slate-200/70 bg-white/85 backdrop-blur-xl">
    <div class="flex min-h-20 flex-col gap-4 px-4 py-4 sm:px-6 lg:px-8 xl:flex-row xl:items-center xl:justify-between">
        <div class="flex min-w-0 items-center gap-4">
            <img src="{{ asset('brand/nexstock-logo.svg') }}" alt="NexStock" class="h-11 w-11 shrink-0 rounded-2xl object-cover shadow-md shadow-blue-950/10">
            <div class="min-w-0">
                <p class="text-xs font-semibold uppercase tracking-normal text-blue-600">NexStock</p>
                <h1 class="truncate text-2xl font-semibold tracking-normal text-slate-950">@yield('title', 'Dashboard')</h1>
            </div>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            <a href="{{ $notificationRoute }}" class="grid h-11 w-11 place-items-center rounded-2xl bg-white text-slate-600 shadow-sm shadow-slate-200 ring-1 ring-slate-200/70 transition hover:text-blue-700 hover:ring-blue-200" aria-label="Notifications">
            ...
            <div class="flex items-center gap-3 rounded-2xl bg-white px-3 py-2 shadow-sm shadow-slate-200 ring-1 ring-slate-200/70">
                ...
                <div class="min-w-0">
                    ...
                    <p class="mt-0.5 inline-flex rounded-full bg-blue-50 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-normal text-blue-700">{{ $user->role }}</p>
                </div>
            </div>
        </div>
    </div>
